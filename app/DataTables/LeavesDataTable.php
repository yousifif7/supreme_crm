...
class LeavesDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->editColumn('hours', fn($leave) => abs($leave->hours))
            ->editColumn('approved_hours', fn($leave) => abs($leave->approved_hours))
            ->editColumn('paid', fn($leave) => $leave->paid ? 'Yes' : 'No')
            ->editColumn('ssp_paid_days', fn($leave) => $leave->ssp_paid_days)
            ->editColumn('unpaid_days', fn($leave) => abs($leave->unpaid_days))

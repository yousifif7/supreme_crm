...
public function dashboardAlerts(Request $request)
{
    ...
    // --- 1. Emergency Alerts ---
    $emergencyAlerts = EmergencyAlert::with('user')
        ->where('acknowledged_by_control', false)
        ->get();
    ...
    // --- 2. Patrol Alerts ---
    $patrols = Patrol::withoutGlobalScope('admin_scope')
        ->where('status', 'pending')
        ->whereHas('shift.staff')
        ->with('shift.staff')
        ->get();
    ...
    foreach ($patrols as $patrol) {
        ...
        $staff = $patrol->shiftDate?->staff;

        $userName = $staff
            ? trim(($staff->first_name ?? '') . ' ' . ($staff->last_name ?? ''))
            : 'Unknown';

        $userId = $patrol->shiftDate?->staff_id ?? 'N/A';
    ...
    // --- 3. Check Call Alerts ---
    $checkCalls = CheckCall::withoutGlobalScope('admin_scope')
        ->where('status', 'pending')
        ->whereHas('shiftDate.staff')
    ...
    foreach ($checkCalls as $checkCall) {
        ...
        $staff = $checkCall->shiftDate?->staff;

        $userName = $staff
            ? trim(($staff->first_name ?? '') . ' ' . ($staff->last_name ?? ''))
            : 'Unknown';

        $userId = $checkCall->shiftDate?->staff_id ?? 0;
    ...
    $shiftDates = ShiftDate::withoutGlobalScope('admin_scope')
        ->whereNull('staff_id')

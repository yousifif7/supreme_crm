    ...
    <form action="" method="post" enctype="multipart/form-data" id="dynamicformsubmit" autocomplete="off">
        ...
        style="width: 530px;" @endif>
                                    <label for="input{{ $input->id }}"
                                        class="form-label @if ($input->required == 1) required @endif">
                                        {{ $input->title }}
                                    </label>

                                    @if (in_array($input->type, ['text', 'email', 'number', 'date', 'password', 'tel', 'url']))
                                        <input id="input{{ $input->id }}" type="{{ $input->type }}"
                                            class="form-control" name="name[{{ $input->id }}]"
                                            placeholder="{{ $input->placeholder }}" value="{{ $input->value }}"
                                            @if ($input->value) readonly 
        style="width: 500px;" @endif>
                                    @elseif($input->type == 'decimal_value')
                                        <input id="input{{ $input->id }}" type="number" step="0.01"
                                            class="form-control" name="name[{{ $input->id }}]"
                                            placeholder="{{ $input->placeholder }}"
                                            value="{{ isset($input->value) ? number_format($input->value, 2, '.', '') : '' }}">
                                    @elseif($input->type === 'textarea')
                                        <textarea id="input{{ $input->id }}" class="form-control" name="name[{{ $input->id }}]"
                                            placeholder="{{ $input->placeholder }}"></textarea>
                                    @elseif($input->type === 'drop')
                                        <select id="input{{ $input->id }}" class="form-select"
                                            name="name[{{ $input->id }}]">
                                            ...
                                            @foreach ($input->child as $option)
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input" type="radio"
                                                        name="name[{{ $input->id }}]" id="radio{{ $option->id }}"
                                                        value="{{ $option->title }}">
                                                    <label class="form-check-label"
                                                        for="radio{{ $option->id }}">{{ $option->title }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                                            ...
                                            @foreach ($input->child as $option)
                                                <div class="form-check">
                                                    <input class="form-check-input" type="checkbox"
                                                        name="name[{{ $input->id }}][]"
                                                        id="checkbox{{ $option->id }}" value="{{ $option->title }}">
                                                    <label class="form-check-label"
                                                        for="checkbox{{ $option->id }}">{{ $option->title }}</label>
                                                </div>
                                            @endforeach
                                        </div>
                                    @elseif($input->type === 'file')
                                        <input id="input{{ $input->id }}" type="file" class="form-control"
                                            name="name[{{ $input->id }}][]" multiple>
                                    @elseif($input->type === 'time')
                                        <input id="datetimepicker{{ $input->id }}" type="text"
                                        ...
                                        <td>
                                            @if (in_array($input->type, ['text', 'email', 'number', 'date', 'password', 'tel', 'url']))
                                                <input type="{{ $input->type }}" class="form-control"
                                                    ...
                                                    placeholder="{{ $input->placeholder }}"@if ($input->value) readonly @endif
                                                    value="{{ $input->value }}">
                                            @elseif($input->type == 'decimal_value')
                                                <input type="number" step="0.01" class="form-control decimal-input"
                                                    ...
                                                    value="{{ isset($input->value) ? number_format($input->value, 2, '.', '') : '' }}">
                                            @elseif($input->type === 'textarea')
                                                <textarea class="form-control" name="name[{{ $input->id }}][{{ $i }}]"
                                                    placeholder="{{ $input->placeholder }}"></textarea>
                                            @elseif($input->type === 'drop')
                                                ...
                                                @foreach ($input->child as $option)
                                                    <div class="form-check form-check-inline">
                                                        <input class="form-check-input" type="radio"
                                                            name="name[{{ $input->id }}][{{ $i }}]"
                                                            value="{{ $option->title }}"
                                                            id="radio{{ $option->id }}_{{ $i }}">
                                                        <label class="form-check-label"
                                                            for="radio{{ $option->id }}_{{ $i }}">{{ $option->title }}</label>
                                                    </div>
                                                @endforeach
                                            @elseif($input->type === 'checkbox')
                                            ...
                                            @elseif($input->type === 'file')
                                                <input type="file" class="form-control"
                                                    name="name[{{ $input->id }}][{{ $i }}][]" multiple>
                                            @elseif($input->type === 'time')
                                                <input type="text"
                                                    id="datetimepicker{{ $input->id }}_{{ $i }}"
                                                    class="form-control datetimepicker" placeholder="Select Time"
                                                    name="name[{{ $input->id }}][{{ $i }}]">
                                            @endif
                                        </td>
                                        ...
                                        <div>
                                            @foreach ($input->child as $option)
                                                <div class="form-check form-check-inline">
                                                    <input class="form-check-input" type="radio"
                                                        name="name[{{ $input->id }}]"
                                                        id="radio{{ $option->id }}" value="{{ $option->title }}">
                                                    <label class="form-check-label"
                                                        for="radio{{ $option->id }}">{{ $option->title }}</label>
                                                </div>
                                            @endforeach
                    ...
                    error: function(xhr) {
                        if (xhr.status === 422) {
                            ...
                            $.each(errors, function(key, value) {
                                ...
                                let inputName = key.replace(/\.(\w+)/g,
                                '[$1]'); // Converts to name[12][0]
                                ...
                                if (inputField.length === 0) {
                                    inputField = $('[name]').filter(function() {
                                        return $(this).attr('name').endsWith(
                                            inputName);
                                    });
                                }
                                ...
                                if (inputField.length > 0) {
                                    ...
                                    console.warn('Field not found for error key:',
                                        inputName);
                                }
                            });

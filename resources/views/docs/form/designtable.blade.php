...
<form action="" method="post" enctype="multipart/form-data" id="dynamicformsubmit" autocomplete="off">
            ...
            <div class="card-body">
                ...
                @foreach($dynamicinput as $input)
                    @if($input->header_status == 1)
                        <div class="col-md-3" @if($input->value) 
        readonly 
        style="width: 530px;" 
    @endif>
                            <label for="input{{$input->id}}" class="form-label @if($input->required == 1) required @endif" >
                                {{ $input->title }}
                            </label>
    ...
    @endif
                            ...
                            @elseif($input->type === 'drop')
                                <select id="input{{$input->id}}" class="form-select" name="name[{{ $input->id }}]">
                                    ...
                                    @foreach($input->child as $option)
                                        <div class="form-check form-check-inline">
                                            <input
                                                ...
                                                value="{{ $option->title }}"
                                            >
                                            <label class="form-check-label" for="radio{{$option->id}}">{{ $option->title }}</label>
                                        </div>
                                    @endforeach
                                </div>
                                    ...
                                    @foreach($input->child as $option)
                                        <div class="form-check">
                                            <input
                                                ...
                                                value="{{ $option->title }}"
                                            >
                                            <label class="form-check-label" for="checkbox{{$option->id}}">{{ $option->title }}</label>
                                        </div>
                                    @endforeach
                                </div>
                ...
                @foreach($dynamicinput as $input)
                    @if($input->header_status == 0)
                        @if($input->type !== 'heading')
                            <td>
                                @if(in_array($input->type, ['text', 'email', 'number', 'date', 'password', 'tel', 'url']))
                                    <input type="{{ $input->type }}" class="form-control" name="name[{{ $input->id }}][{{ $i }}]" placeholder="{{ $input->placeholder }}"@if($input->value) readonly @endif value="{{$input->value}}">
                                @elseif($input->type == 'decimal_value')
                                    <input
                                        type="number"
                                        step="0.01"
                                        class="form-control decimal-input"
                                        name="name[{{ $input->id }}][{{ $i }}]"
                                        placeholder="{{ $input->placeholder }}"
                                        value="{{ isset($input->value) ? number_format($input->value, 2, '.', '') : '' }}"
                                    >
                                @elseif($input->type === 'textarea')
                                    <textarea class="form-control" name="name[{{ $input->id }}][{{ $i }}]" placeholder="{{ $input->placeholder }}"></textarea>
                                @elseif($input->type === 'drop')
                                    <select class="form-select" name="name[{{ $input->id }}][{{ $i }}]">
                                        <option value="">{{ $input->title }}</option>
                                        @foreach($input->child as $option)
                                            <option value="{{ $option->id }}">{{ $option->title }}</option>
                                        @endforeach
                                ...
                                @elseif($input->type === 'radio')
                                    @foreach($input->child as $option)
                                        <div class="form-check form-check-inline">
                                            <input class="form-check-input" type="radio" name="name[{{ $input->id }}][{{ $i }}]" value="{{ $option->title }}" id="radio{{ $option->id }}_{{ $i }}">
                                            <label class="form-check-label" for="radio{{ $option->id }}_{{ $i }}">{{ $option->title }}</label>
                                        </div>
                                    @endforeach
                                @elseif($input->type === 'checkbox')
                                    @foreach($input->child as $child)
                                        <div class="form-check">
                                            <input class="form-check-input" type="checkbox" name="name[{{ $input->id }}][{{ $i }}][]" value="{{ $child->title }}">
                                            <label class="form-check-label">{{ $child->title }}</label>
                                        </div>
                                    @endforeach
                                @elseif($input->type === 'file')
                                    <input type="file" class="form-control" name="name[{{ $input->id }}][{{ $i }}][]" multiple>
                                @elseif($input->type === 'time')
                                    <input type="text" id="datetimepicker{{ $input->id }}_{{ $i }}" class="form-control datetimepicker" placeholder="Select Time" name="name[{{ $input->id }}][{{ $i }}]">
                                @endif
                            </td>
                        @endif
                    @endif
...
<div class="card">
    <div class="card-body">
            ...
            <div class="row d-flex justify-content-center" style="background-color:yellow">
                <div class="col-md-6">
                    <p class="p-1 m-0" style="text-align:center"><strong>I'm Responsible for paying my own National insurance and Tax.</strong></p>
                </div>
            </div>
    </div>
</div> 
    ...
    <div class="form-section d-none extra-desc">{!! $digitalform->desc !!}</div>
        ...
        @if($digitalform->paginate_status == 1)
            <div class="form-navigation d-flex align-items-center justify-content-between flex-wrap mt-3">
                ...
                <button type="submit" id="submitForm" style="display:none;" class="btn btn-success mb-2">Submit</button>
            </div>
        @else
...
error: function (xhr) {
    if (xhr.status === 422) {
        ...
        $.each(errors, function (key, value) {
            // Laravel returns errors like "name.12" or "name.12.0"
            let inputName = key.replace(/\.(\w+)/g, '[$1]'); // Converts to name[12][0]

            // Try direct match
            let inputField = $('[name="' + inputName + '"]');

            // Fallback: search any input that *ends* with the key (to catch dynamically generated names)
            if (inputField.length === 0) {
                inputField = $('[name]').filter(function () {
                    return $(this).attr('name').endsWith(inputName);
                });
            }
            ...
            if (inputField.length > 0) {
                ...
                console.warn('Field not found for error key:', inputName);
            }
        });
        ...
        // Show the correct page if paginated
        if (typeof findFirstErrorPage === 'function') {
        ...
        console.error('Other error:', xhr.responseJSON);
    }
}
    ...
    $(".datetimepicker").each(function () {
        let picker = $(this).flatpickr({
            ...
            onClose: function(selectedDates, dateStr, instance) {
                // instance.close(); ❌ Remove this line

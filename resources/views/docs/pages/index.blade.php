                    ...
                    <table id="myTable" class="table datanew" data-update-route="{{ route('digital.form.order') }}">
                        <thead>
                            <tr>
                                ...
                                @php
                                    $hasActionPermission =
                                        auth()->user()->can('page edit') || auth()->user()->can('page delete');
                                @endphp
                                <th>Action</th>
                            </tr>
                        </thead>
                        <tbody id="tablecontents">
                            @foreach (App\Models\Docs\Page::get() as $key => $item)
                                @php
                                    $digiform = App\Models\Docs\DigitalForm::where(
                                        'id',
                                        $item->select_form_id,
                                    )->first();
                                ...
                                <tr class="row1" data-id="{{ $item->id }}">
                                    ...
                                    <td>
                                        <a class="btn btn-xs btn-info btn-change" target="_blank"
                                            href="{{ route('page.form.design', $item->slug) }}">
                                            <i class="fa-solid fa-eye"></i>
                                        </a>
                                        @if ($hasActionPermission)
                                            @can('page edit')
                                                <a class="btn btn-xs btn-success btn-change"
                                                    href="{{ url('page/form/edit' . '/' . $item->id) }}">
                                                    <i class="fa-solid fa-pen-to-square"></i>
                                                </a>
                                            @endcan
                                            @can('page delete')
                                                <form method="POST" action="{{ route('page.form.destroy', $item->id) }}"

            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="myTable" class="table datanew"
                        data-update-route="{{ route('digital.form.field.order') }}">
                        <thead>
                            <tr>
                                <th>#</th>
                                <th>title</th>
                                ...
                                @php
                                    $hasActionPermission =
                                        auth()->user()->can('dynamicinput edit') ||
                                    ...
                                    @if ($item->type == 'checkbox' || $item->type == 'drop' || $item->type == 'radio')
                                        <td>
                                            <input type="checkbox" class="status-toggle" data-id="{{ $item->id }}"
                                                id="switch{{ $item->id }}" switch="danger"
                                        ...
                                        <td>
                                            @can('dynamicinput edit')
                                                <a class="btn btn-xs btn-success btn-change"
                                                    href="{{ url('digital/form/field/edit' . '/' . $item->id) }}">
                                            ...
                                            @can('dynamicinput')
                                                @if ($item->type == 'checkbox' || $item->type == 'drop' || $item->type == 'radio')
                                                    <a class="btn btn-xs btn-info btn-change"
                                                        href="{{ url('digital/form/field/child' . '/' . $item->id) }}">
                                            ...
                                            @can('dynamicinput delete')
                                                <form method="POST"
                                                    action="{{ route('digital.form.field.destroy', $item->id) }}"

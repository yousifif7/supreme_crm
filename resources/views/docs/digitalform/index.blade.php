                    ...
                    <table id="myTable" class="table datanew" data-update-route="{{ route('digital.form.order') }}">
                        ...
                        <tbody id="tablecontents">
                            ...
                            <tr class="row1" data-id="{{ $item->id }}">
                                ...
                                <td>{{$item->title}}</td>
                                <td>
                                    <input type="checkbox" class="status-toggle-paginate" data-id="{{ $item->id }}" id="switch{{ $item->id }}"
                                ...
                                <td>
                                    ...
                                    <a class="btn btn-xs btn-success btn-change" href="{{url('digital/form/edit'.'/'.$item->id)}}">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>
                                    @endcan
                                    @can('dynamicinput')
                                    <a class="btn btn-xs btn-info btn-change" href="{{url('digital/form/field'.'/'.$item->id)}}">
                                        <i class="fa-solid fa-bars"></i>
                                    </a>
                                    @endcan
                                    @can('digitalform delete')
                                    <form method="POST" action="{{ route('digital.form.destroy', $item->id) }}" style="display: contents;">
                                        @csrf

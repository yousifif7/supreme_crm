                ...
                <div class="form-section" @if ($input->type === 'heading') style="padding:0" @endif>
                    @if ($input->type === 'heading')
                        <div class="card-header"
                            @if ($input->type === 'heading') style="background-color: rgb(223, 199, 0);
    padding: 13px 18px 4px 8px" @endif>
                            <h3
                                @if ($input->type === 'heading') style="font-weight: 600;text-decoration: underline;font-size:15px" @endif>
                                {{ $input->title }}</h3>
                        </div>
                        @if ($input->desc)
                    ...
                    @if (in_array($input->type, ['text', 'email', 'number', 'date', 'password', 'tel', 'url']))
                        <input type="{{ $input->type }}" class="form-control" name="name[{{ $input->id }}]"
                            placeholder="{{ $input->placeholder }}" value="{{ $decodedData[$input->id] ?? '' }}">
                    ...
                    @elseif($input->type === 'drop')
                        <select class="form-select" name="name[{{ $input->id }}]">
                            ...
                            @foreach ($input->child as $option)
                                <option value="{{ $option->id }}" @if (isset($decodedData[$input->id]) && $decodedData[$input->id] == $option->id) selected @endif>
                                    {{ $option->title }}</option>
                            @endforeach
                        </select>
                    @elseif($input->type === 'radio' && !empty($input->child))
                        @foreach ($input->child as $option)
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="name[{{ $input->id }}]"
                                    value="{{ $option->title }}" @if (isset($decodedData[$input->id]) && $decodedData[$input->id] == $option->title) checked @endif>
                        ...
                        @foreach ($input->child as $child)
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="name[{{ $input->id }}][]"
                                    value="{{ $child->title }}" @if (isset($decodedData[$input->id]) && in_array($child->title, $decodedData[$input->id])) checked @endif>
                    ...
                    @elseif($input->type === 'time')
                        <input type="text" id="datetimepicker{{ $input->id }}"
                            ...
                            name="name[{{ $input->id }}]" value="{{ $decodedData[$input->id] ?? '' }}">
                    @endif
                </div>
            @endforeach

   ...
   <div class="row mb-4">
    ...
    @endif>
                    ...
                    @if(in_array($input->type, ['text', 'email', 'number', 'date', 'password', 'tel', 'url']))
                        <input type="{{ $input->type }}" class="form-control" name="{{ $inputName }}" value="{{ $value }}" placeholder="{{ $input->placeholder }}" 
    ...
    @endif>
                    @elseif($input->type == 'decimal_value')
                        <input type="number" step="0.01" class="form-control" name="{{ $inputName }}" value="{{ $value }}" placeholder="{{ $input->placeholder }}">
                    ...
                    @elseif($input->type === 'radio')
                        @foreach($input->child as $option)
                            <div class="form-check form-check-inline">
                                <input class="form-check-input" type="radio" name="{{ $inputName }}" value="{{ $option->title }}" {{ $value == $option->title ? 'checked' : '' }}>
                    ...
                    @elseif($input->type === 'checkbox')
                        ...
                        @foreach($input->child as $option)
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" name="{{ $inputName }}[]" value="{{ $option->title }}" {{ in_array($option->title, $checkedValues) ? 'checked' : '' }}>
                                <label class="form-check-label">{{ $option->title }}</label>
                            </div>
                        @endforeach
                    @elseif($input->type === 'file')
                        <input type="file" class="form-control" name="{{ $inputName }}[]" multiple>
                    @elseif($input->type === 'time')
                        <input type="time" class="form-control datetimepicker" name="{{ $inputName }}" value="{{ $value }}">
                    @endif
                </div>
            @endif
        @endforeach
            ...
            @if($input->header_status == 2)
                <div class="col-md-2">
                    ...
                    @elseif($input->type === 'file')
                        <input type="file" class="form-control" name="{{ $inputName }}[]" multiple>
                    @elseif($input->type === 'time')
                        <input type="time" class="form-control" name="{{ $inputName }}" value="{{ $value }}">
                    @endif
                </div>
            @endif
        @endforeach

                        ...
                        <form method="GET" action="{{ route('form_detail.index', ['id' => $digi->id]) }}">
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    ...
                                    <a href="{{ route('form_detail.index', $digi->id) }}"
                    ...
                    <table id="sortableTable" class="table">
                        <thead>
                            <tr>
                                @php
                                    $hasActionPermission =
                                        auth()->user()->can('client detail destroy') ||
                                        auth()->user()->can('client detail edit') ||
                                        auth()->user()->can('client detail show only');
                                    $dynamicFields = App\Models\Docs\DynamicInputs::where('child_id', 0)
                                        ->where('parent_id', $digi->id)
                                        ->distinct()
                                        ->get();
                                @endphp
                                    ...
                                    @if ($hasActionPermission)
                                        <td>
                                            @can('client detail edit')
                                                <a class="btn btn-xs btn-success editBtn btn-change"
                                                    href="javascript:void(0);" data-id="{{ $input->id }}">
                                                    <i class="fa-solid fa-pen"></i>
                                                </a>
                                            @endcan

                                            <a class="btn btn-xs btn-info btn-change"
                                                href="{{ route('client.complete.detail.index', $input->id) }}">
                                                <i class="fa-solid fa-eye"></i>
                                            </a>

                                            @can('client detail destroy')
                                                <form method="POST"
                                                    action="{{ route('clientdetail.destroy', $input->id) }}"
                                        ...
                                        <td>
                                            @if (Str::startsWith($value, 'uploads/'))
                                                <a href="{{ asset($value) }}" target="_blank">
                                                    <img src="{{ asset($value) }}" alt="Uploaded Image" width="50"

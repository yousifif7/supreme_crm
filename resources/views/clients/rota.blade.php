@extends('layouts.app')
@section('title', brand_title('Site Calendar'))
@section('styles')
    <style>
...
._schedule-box-container {
    ...
    cursor: default; /* non-clickable for clients */
    line-height: 1.3;
...
@section('contents')<!-- Page Wrapper -->
    <div id="scheduling" class="page-wrapper site_calendar">
        <div class="content">
            ...
            {{-- Simplified header for client rota: no tabs to other rotas --}}
            <div class="d-flex align-items-baseline justify-content-between flex-wrap gap-1 mb-3">
                <div class="left">
                    <h4>Client Rota</h4>
                </div>
                <div class="right">
                    <button class="refresh_btn btn btn-white" onclick="window.location.reload()">
            ...
            <!-- Add shift -->
            <!-- client rota: do not include shift modal or assign-shift modal -->
            <!-- /Breadcrumb -->
    ...
    <!-- Assign Shift Modal (removed for client view) -->
    </div>
...
eventContent: function(info) {
    ...
    return {
        domNodes: [container]
                        ...
                        // Disable clicks on events for client view: do not open shift details
                        eventClick: function(info) {
                            // prevent any default click behavior for clients
                            if (info.jsEvent) info.jsEvent.preventDefault();
                            return false;
                        },

                        eventDidMount: function(info) {
                            // make the event non-interactive
                            info.el.style.backgroundColor = 'transparent';
                            info.el.style.border = 'none';
                            info.el.style.overflow = 'visible';
                            if (info.el.style) info.el.style.pointerEvents = 'none';
                        }
                    });

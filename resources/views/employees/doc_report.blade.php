                    ...
                    <div class="row">
                        <div class="col-md-3">
                            ...
                            <select name="document_field[]" id="document_field" class="form-select" multiple>
                                @php
                                    // Include driving_licence in documentFields if not already
                                @endphp
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @php
                                                    ...
                                                    $uploadedCount = 0;
                                                    $totalCount = count($selectedFields);
                                                @endphp

                                                @foreach ($selectedFields as $field)
                                                    @if ($employee->{$field})
                                                        <span class="badge bg-success">{{ $documentFields[$field] ?? $field }} Uploaded</span>
                                                        @php $uploadedCount++; @endphp
                                                    @else
                                                        <span class="badge bg-danger">{{ $documentFields[$field] ?? $field }} Missing</span>
                                                    @endif
                                                @endforeach
                                            ...
                                            <td>
                                                @php
                                                    $expiryDates = [];
                                                    foreach ($selectedFields as $field) {
                                                        if (array_key_exists($field, $expiryFields) && $employee->{$field}) {
                                            ...
                                            <td>
                                                @if ($expiryDates)
                                                    @foreach ($expiryDates as $field => $date)
                                                        @php
                                                            $daysRemaining = $date ? now()->diffInDays(\Carbon\Carbon::parse($date), false) : null;
                                                        @endphp
                                                        @if ($daysRemaining !== null)
                                                            @if ($daysRemaining > 0)
                                                                <span class="badge bg-success">{{ round($daysRemaining) }} days</span>
                                                            @else
                                                                <span class="badge bg-danger">Expired {{ abs(round($daysRemaining)) }} days ago</span>
                                                            @endif
                                                        @endif
                                                    @endforeach
                                                @else
                                            ...
                                            <td>
                                                <a href="{{ url('employees#' . $employee->id) }}" class="btn btn-sm btn-primary">View</a>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>

...
@section('contents')
    <div id="all-workers" class="page-wrapper">
                            ...
                            <div class="col-md-3">
                                ...
                                <select name="document_field" id="document_field" class="form-select">
                                    ...
                                    @foreach ($documentFields as $field => $label)
                                        <option value="{{ $field }}" {{ $documentField == $field ? 'selected' : '' }}>
                                            {{ $label }}
                                        </option>
                                    @endforeach
                                </select>
                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($documentField && array_key_exists($documentField, $expiryFields) && $employee->{$documentField} && $expiryDate)
                                                        ...
                                                        @else
                                                            <span class="badge bg-danger">Expired {{ abs(round($daysRemaining, 0)) }} days ago</span>
                                                        @endif
                                                    @else
                                                        N/A
                                                    @endif
        ...
        </div> <!-- ✅ this closes .content properly -->
    </div>
@endsection
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function updateExpiryStatusDisabledState() {
                ...
                if (selectedField && @json(array_keys($expiryFields)).includes(selectedField) && uploadStatus !==
                    'missing') {
